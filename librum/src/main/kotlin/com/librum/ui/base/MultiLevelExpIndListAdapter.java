...
/**
 * Multi-level expandable indentable list adapter. Initially all elements in the list are single
 * items. When you want to collapse an item and all its descendants call {@link
 * #collapseGroup(int)}. When you want to exapand a group call {@link #expandGroup(int)}. Note that
 * groups inside other groups are kept collapsed.
 *
 * To collapse an item and all its descendants or expand a group at a certain position you can call
 * {@link #toggleGroup(int)}.
 *
 * To preserve state (i.e. which items are collapsed) when a configuration change happens (e.g.
 * screen rotation) you should call {@link #saveGroups()} inside onSaveInstanceState and save the
 * returned value into the Bundle. When the activity/fragment is recreated you can call {@link
 * #restoreGroups(List)} to restore the previous state. The actual data (e.g. the comments in the
 * sample app) is not preserved, so you should save it yourself with a static field or implementing
 * Parcelable or using setRetainInstance(true) or saving data to a file or something like that.
 *
...
public abstract class MultiLevelExpIndListAdapter extends RecyclerView.Adapter {
   ...
   *
   * @param item The item or group to be removed.
   * @param expandGroupBeforeRemoval True to expand the group before removing the item. False to
   * remove also all the items and groups contained if the item to be removed is a group.
   * @return true if this adapter was modified by this operation, false otherwise.
   */
  ...
  /**
   * In onSaveInstanceState, you should save the groups' indices returned by this function in the
   * Bundle so that later they can be restored using {@link #restoreGroups(List)}. saveGroups()
  ...
  public interface ExpIndData {
    ...
    /**
     * @param groupSize Set the number of items in the group. Note: groups contained in other groups
     * are counted just as one, not as the number of items that they contain.
     */
